import type { ModrinthProject, ModrinthSearchResponse, ModrinthVersion } from '../main/mods/modrinthClient';
import type { InstalledModRecord, ModsPersistedState } from '../main/mods/modsTypes';
import type { LauncherProfile } from '../main/profiles/profileTypes';
...
export type ModsFail = { ok: false; title: string; detail: string };
...
export type ModsSearchResponse = { ok: true; result: ModrinthSearchResponse } | ModsFail;
export type ModsProjectResponse = { ok: true; project: ModrinthProject } | ModsFail;
export type ModsVersionsResponse = { ok: true; versions: ModrinthVersion[] } | ModsFail;
export type ModsVersionResponse = { ok: true; version: ModrinthVersion } | ModsFail;
export type ModsInstallVersionResponse = { ok: true; mod: InstalledModRecord; installedDeps: string[] } | ModsFail;
export type ModsResolveLatestResponse = { ok: true; version: ModrinthVersion } | ModsFail;
export type ModsImportPathsResponse = { ok: true; mods: InstalledModRecord[] } | ModsFail;

export type ProfilesListResponse = { ok: true; profiles: LauncherProfile[] };
export type ProfileMutationResponse = { ok: true; profile: LauncherProfile } | { ok: false; title: string; detail: string };
export type ProfileActionResponse = { ok: true } | { ok: false; title: string; detail: string };
...
export type VexLauncherApi = {
	...
	modsGetState: () => Promise<ModsGetStateResponse>;
	modsSearch: (opts: {
		query: string;
		limit?: number;
		offset?: number;
	...
	}) => Promise<ModsSearchResponse>;
	modsGetProject: (id: string) => Promise<ModsProjectResponse>;
	modsListProjectVersions: (projectId: string, gameVersion?: string, loader?: string) => Promise<ModsVersionsResponse>;
	modsGetVersion: (versionId: string) => Promise<ModsVersionResponse>;
	modsInstallVersion: (opts: {
		versionId: string;
		minecraftVersion: string;
		loader: ModLoader;
		installDeps: boolean;
	}) => Promise<ModsInstallVersionResponse>;
	modsResolveLatest: (opts: { projectId: string; minecraftVersion: string; loader: ModLoader }) => Promise<ModsResolveLatestResponse>;
	...
	modsImportPaths: (opts: { paths: string[]; minecraftVersion: string; loader: ModLoader }) => Promise<ModsImportPathsResponse>;
	modsSetProfileSelection: (patch: { selectedModLoader?: ModLoader; selectedProfileId?: string | null }) => Promise<{ ok: true }>;
	profilesList: () => Promise<ProfilesListResponse>;
	profilesSetActive: (profileId: string) => Promise<ProfileActionResponse>;
	profilesCreate: (input: Partial<LauncherProfile>) => Promise<ProfileMutationResponse>;
	profilesUpdate: (opts: { id: string; patch: Partial<LauncherProfile> }) => Promise<ProfileMutationResponse>;
	profilesDelete: (profileId: string) => Promise<ProfileActionResponse>;
	profilesDuplicate: (profileId: string) => Promise<ProfileMutationResponse>;
};
